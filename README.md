...
| `set uts_prepCodpiece = true` | After the run (and after the banish rundown, if that's enabled, so its farmed pearls count), load The Eternity Codpiece for your next ascension: empties Hagnk's first (`pull all`), buys unblemished pearls from the mall if you're still short, then slots five of them. |
| `set uts_runOutEagleBanish = true` | **Experimental.** After the run finishes, re-aim the patriotic eagle's Patriotic Screech off the construct phylum — a leftover construct banish can make other scripts misbehave. The screech recharges after 11 combats with the eagle out, and an unblemished pearl at 18+ resistance is ten combats, so the two finish together: the script empties Hagnk's (`pull all`), then farms a pearl with the eagle (an open pearl zone with today's pearl unclaimed, wearing that zone's elemental resistance) until the screech is back, finishing any pearl in progress (zone progress doesn't survive rollover). It then banishes the first monster at the Smut Orc Logging Camp, which moves the lingering banish onto the orc phylum and frees constructs immediately — about a dozen post-run turns, and the pearl rides to your next ascension in the codpiece. Needs the eagle, a pearl zone, the Logging Camp open, and turns of Fishy; aborts loudly the moment it can't continue. |

...
| `set uts_postLoopRunOutEagleBanish = true` | **Experimental.** After the run finishes, keep adventuring until the patriotic eagle's Patriotic Screech stops banishing the construct phylum, which can make other scripts (⭐garbo⭐) misbehave. Empties Hagnk's first, then burns the recharge turns farming unblemished pearls before we screech in the Smut Orc Logging Camp; with no pearl zone available it recharges in the camp instead. |

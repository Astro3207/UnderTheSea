...
| `set uts_postLoopRunOutEagleBanish = true` | **Experimental.** After the run finishes, keep adventuring until the patriotic eagle's Patriotic Screech stops banishing the construct phylum, which can make other scripts (⭐garbo⭐) misbehave. Empties Hagnk's first, then farms unblemished pearls -- regardless of `uts_postLoopFarmPearls` -- until the screech recharges, and spends it in the Smut Orc Logging Camp. With no pearl zone left to farm it stops and reports that constructs are still banished, rather than aborting the rest of the postloop. |

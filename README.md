...
| `set uts_postLoopRunOutEagleBanish = true` | **Experimental.** After the run finishes, keep adventuring until the patriotic eagle's Patriotic Screech stops banishing the construct phylum, which can make other scripts (⭐garbo⭐) misbehave. Empties Hagnk's first, then burns the recharge turns farming unblemished pearls before we screech in the Smut Orc Logging Camp. When no pearl zone is usable -- today's pearls already claimed, or Fishy gone -- the recharge happens in the Smut Orc Logging Camp instead, so the re-aim no longer depends on there being pearls left to farm. |
